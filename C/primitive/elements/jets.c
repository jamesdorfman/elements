...
/* Write an optional confidential asset rangeproof hash from an 'assetIssuance' to the 'dst' frame, advancing the cursor 257 cells.
 *
 * Precondition: '*dst' is a valid write frame for 257 more cells;
 *               NULL != issuance;
 */
...
/* Write an optional confidential token rangeproof hash from an 'assetIssuance' to the 'dst' frame, advancing the cursor 257 cells.
 *
 * Precondition: '*dst' is a valid write frame for 257 more cells;
 *               NULL != issuance;
 */
...
/* output_surjection_proof : TWO^32 |- S (S TWO^256) */
bool output_surjection_proof(frameItem* dst, frameItem src, const txEnv* env) {
  uint_fast32_t i = read32(&src);
  if (writeBit(dst, i < env->tx->numOutputs)) {
    if (writeBit(dst, is_confidential(env->tx->output[i].asset.prefix))) {
...
/* output_range_proof : TWO^32 |- S (S TWO^256) */
bool output_range_proof(frameItem* dst, frameItem src, const txEnv* env) {
  uint_fast32_t i = read32(&src);
  if (writeBit(dst, i < env->tx->numOutputs)) {
    if (writeBit(dst, is_confidential(env->tx->output[i].amt.prefix))) {
