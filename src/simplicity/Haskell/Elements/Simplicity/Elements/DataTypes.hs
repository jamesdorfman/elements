...
putAsset (Asset (Confidential (Point by x) _)) = putWord8 (if by then 0x0b else 0x0a) >> put x
...
putAmount (Amount (Confidential (Point by x) _)) = putWord8 (if by then 0x09 else 0x08) >> put x
...
instance Serialize Nonce where
  ...
  put (Nonce (Confidential (Point by x) _)) = putWord8 (if by then 0x03 else 0x02) >> put x
  get = lookAhead getWord8 >>= go
   where
    go 0x01 = getWord8 *> (Nonce . Explicit <$> get)
    go 0x02 = Nonce . flip Confidential () . Point False <$> get
    go 0x03 = Nonce . flip Confidential () . Point True <$> get
